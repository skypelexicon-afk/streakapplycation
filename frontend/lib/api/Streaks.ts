...
export const getMyStreak = async (): Promise<Streak> => {
    const response = await doFetch(`${API_BASE_URL}/api/streaks/my-streak`);
...
export const updateStreak = async (): Promise<{
    ...
    const response = await doFetch(`${API_BASE_URL}/api/streaks/update`, {
        method: 'POST',
    });
    return response;
};
...
export const getAllBadges = async (): Promise<Badge[]> => {
    const response = await doFetch(`${API_BASE_URL}/api/streaks/badges`);
    return response.badges;
};

// Get user's earned badges
export const getMyBadges = async (): Promise<Badge[]> => {
    const response = await doFetch(`${API_BASE_URL}/api/streaks/my-badges`);
    return response.badges;
};
